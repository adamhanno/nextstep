...
const HabitTracker = () => {
    ...
    const fetchHabits = async () => {
        ...
        const journalEntriesResponse = await axios.get('http://localhost:5000/api/habits/journalEntries', {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
    ...
    const addHabit = async (e) => {
        e.preventDefault();
        const token = localStorage.getItem('token');
        await axios.post('http://localhost:5000/api/habits', { name: habitName, frequency, goal }, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
    ...
    const startHabitTimer = (id) => {
        if (timers.current[id] || habits.find(h => h._id === id)?.progress >= 100) return;
    ...
    const updateHabitProgress = async (id, startTime) => {
        ...
        const updatedTime = Math.min(habit.durationCompleted + elapsedTime, habit.goal);
        ...
        <div className="habit-tracker-container">
            ...
            <ul className="habit-list">
                {habits.map((habit) => (
                    <li key={habit._id}>
                        <span>
                            {habit.name} - Progress: {(habit.progress || 0).toFixed(2)}% | Goal: {habit.goal || 0} mins
                        </span>
                        ...
                        <textarea
                            ...
                            value={journalEntries[habit._id] || ''} // Set journal entry per habit
                            onChange={(e) => handleJournalEntryChange(habit._id, e.target.value)}
                            disabled={!isEditing[habit._id]} // Disable if not editing
                        />
                        {!isEditing[habit._id] ? (
                            <button onClick={() => enableEdit(habit._id)}>Edit Journal</button>
                        ) : (
                            <button onClick={() => saveJournalEntry(habit._id)}>Save Journal Entry</button>
                        )}
                    </li>
                ))}
            </ul>

            <button className="toggle-visualizations" onClick={() => setShowVisualizations(!showVisualizations)}>
